...
def make_qr(input: str, password: str):
    ...
    #header variable that contains the encryption scheme and password information
    header = f"ENCRYPTION_SCHEME:AES_GCM,password:{password}"

    #concatenate the salt, token, and header variables to create the final blob
    blob = salt + token + header.encode()
...
def read_qr(qr, password):
    ...
    blob = base64.urlsafe_b64decode(data.encode())

    header_length = 32  # Assuming the header is at most 32 bytes long
    header = blob[:header_length]
    encrypted_data = blob[header_length:]
